...
def test_cloud_manager():
    ...
    def mocked_add_member_to_group(member_email, group_id):
        response = {}
        if group_id == 'test_bucket' and member_email == 'user0_proxy_group@example.com':
            response = {'email': 'user0_proxy_group@example.com'}
        else:
            raise Exception(
                'cannot add {} to group {}'
                .format(member_email, group_id)
            )
        return response
    ...
    manager.return_value.__enter__.return_value.get_group = (
        mocked_get_group)
    ...
    patch('storageclient.google.GoogleCloudManager', manager).start()
    return manager
